...
import Tableview,{NepalView,WorldView} from './Tableview'
  ...
  class Homepage extends Component {
    ...
    //   fetch(urlnp).then(resp => resp.json().then(datan => this.setState({npcondition:datan}))).then(r =>console.log(JSON.stringify(this.state.npcondition)));
     

    //   const urlw = 'https://wuhan-coronavirus-api.laeyoung.endpoint.ainize.ai/jhu-edu/brief';
    //   fetch(urlw).then(resp => resp.json().then(dataw => this.setState({worldcondition:dataw}))).then(r =>console.log(JSON.stringify(this.state.worldcondition)));
    ...
    render(){
      ...
      <h2>{this.state.lan==='np'?'सर्वेक्षण समीक्षा:':'Survey Overview'}</h2>
