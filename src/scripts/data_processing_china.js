...
lineReader.on('line', function(line) {
    ...
    let city = lineSplit[11]
    let cityEnglish = lineSplit[12]
    let cityConfirmedCount = parseInt(lineSplit[14], 10)
    let cityCuredCount = parseInt(lineSplit[16], 10)
    let cityDeadCount = parseInt(lineSplit[17], 10)
    let date = lineSplit[18].substr(0, 10)

    // fix format inconsistency (https://github.com/BlankerL/DXY-COVID-19-Data/issues/55)
    if ((date.length !== 10 || date[4] !== '-') && date !== 'updateTime') {
        date = lineSplit[11].substr(0, 10)
        city = lineSplit[12]
        cityEnglish = lineSplit[13]
        cityConfirmedCount = parseInt(lineSplit[15], 10)
        cityCuredCount = parseInt(lineSplit[17], 10)
