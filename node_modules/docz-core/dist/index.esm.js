...
import { resolver, defaultHandlers, parse as parse$1 } from 'react-docgen';
...
const execDevCommand = async ({
  ...
  const gatsbyPath = path$1__default.join(repoRootPath, 'node_modules/.bin/gatsby');
...
class Entries {
  ...
  async getMap(config) {
    ...
    const globIgnore = config.src.indexOf('node_modules') === -1 ? ['**/node_modules/**'] : [];
    const fileMatchingPatterns = getFilesToMatch(config); // Hack around fast-glob not returning the whole set when many patterns are provided in the array
...
const jsParser = (files, config) => {
  const resolver$1 = config.docgenConfig.resolver || resolver.findAllExportedComponentDefinitions;
  const root = getRootDir(config);
